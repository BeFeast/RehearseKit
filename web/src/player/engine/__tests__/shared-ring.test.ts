...
import { createRingLayout, CTRL, STATE, MAX_STEMS, CTRL_BYTES, METER_BYTES } from '../ring-layout';
...
describe('createRingLayout', () => {
  it('lays out planar channels after the control block', () => {
    ...
    const base = CTRL_BYTES + METER_BYTES;
    expect(layout.meterByteOffset).toBe(CTRL_BYTES);
    expect(layout.stems[0].channelByteOffsets).toEqual([base, base + 32]);
    expect(layout.stems[1].channelByteOffsets).toEqual([base + 64]);
    expect(layout.totalBytes).toBe(base + 96);
