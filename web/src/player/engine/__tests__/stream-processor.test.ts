...
import { createRingLayout, CTRL, METER_FLOATS, STATE } from '../ring-layout';
...
beforeAll(async () => {
  ...
  // @ts-expect-error plain-JS worklet module, evaluated once the fakes are in place
  await import('../../../../public/stream-processor.js');
});
...
function setup(stemChannels: number[], ringFrames = 512) {
  ...
  const proc = new Processor({
    processorOptions: { sab, layout, ctrl: CTRL, state: STATE, meter: { floats: METER_FLOATS, releaseDbPerS: 20, holdSeconds: 1.5 } },
  });
