...
| Real-time processing stage/progress updates | **Implemented** (server side; corrected 2026-08-01) | `backend/app/tasks/audio_processing.py` publishes Redis updates and `frontend/utils/websocket.ts`, `frontend/components/job-card.tsx`, and `frontend/app/jobs/[id]/page.tsx` contain a client. **Correction 2026-08-01:** the original audit claimed no checked-in WebSocket server exists; that was false. `websocket/app/main.py` has existed since the initial commit and implements the Redis subscriber exposing `/ws/jobs/{job_id}/progress` plus a `/health` endpoint, and the root `docker-compose.yml` builds it from `./websocket`. Only the audited TrueNAS compose file references a pre-published image. The job list polls every five seconds in `frontend/components/processing-queue.tsx`, so status can still refresh without real-time delivery. The PRD's SSE fallback is absent. | WebSocket client behavior is unit-tested in `frontend/utils/__tests__/websocket.test.ts`; `frontend/e2e/complete-flow.spec.ts` contains an assertion-free WebSocket placeholder. |
...
- ~~The repository contains a WebSocket client and Redis publisher but no checked-in WebSocket server implementation, so the real-time path cannot be rebuilt from source.~~ **Correction 2026-08-01:** this claim was false. The WebSocket server is checked in at `websocket/app/main.py` (present since the initial commit) and is built from source by the root `docker-compose.yml` (`build: ./websocket`). What remains true is that the server has no automated tests.
...
2. ~~**Restore a source-built WebSocket service.** Add the missing Redis subscriber/WebSocket server, health endpoint, Docker build target, and an integration test for one published progress update.~~ **Struck 2026-08-01:** this proposal was based on a false audit claim. The source-built WebSocket service already exists (`websocket/app/main.py`: Redis subscriber, `/ws/jobs/{job_id}/progress`, `/health`; root `docker-compose.yml` builds `./websocket`). Do not promote this to an issue. The only residual gap is test coverage, which belongs to proposal 4's compose smoke check.
